...
// import { getStorage, ref, uploadBytesResumable,downloadURL,getDownloadURL } from "firebase/storage";
import {
  getDownloadURL,
  getStorage,
...
export default function CreateListing() {
  ...
  const handleImageSubmit = (e) => {
    if (files.length > 0 && files.length + formData.imageUrls.length < 7) {
      ...
      setImageUploadError("Maximum image upload is 6 images per listing");
      setUploading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (+formData.discountPrice > +formData.regularPrice)
        return setError("Don't know what discount means?");
      setLoading(true);
      setError(false);
